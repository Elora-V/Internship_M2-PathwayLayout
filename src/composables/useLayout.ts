 ...
 * @param [callbackFunction=() => {}] function to do after the layout is done
 */
export async function vizLayout(subgraphNetwork:SubgraphNetwork,assignRank:boolean=false, cycle:boolean=true,callbackFunction = () => {}): Promise<void> {
    console.log('Viz');
    await instance().then( async viz => {
        const graphViz=NetworkToViz(subgraphNetwork,cycle);
        const dot =NetworkToDot(graphViz)
        //console.log(dot);
