...
export async function sbml2json(sbmlFilePath: string): Promise<JsonGraphFormat> {
    try {
        // read xml file
        const response = await fetch(sbmlFilePath);
        // DATA
        const sbmlString: string = await response.text();
        ...
        console.log('could not fetch the xml file !');
        console.log(error);
        return null;
    }
}
